...
-- select: Consulta SQL
select c.modelo_carro, m.nombre_marca, ge.nombre_grupo, c.fecha_compra, c.matricula, c.color, c.km_totales, 
a.nombre_aseguradora, (p.id)poliza
from flotavehiculos.carros c join flotavehiculos.poliza p on c.num_poliza = p.id
join flotavehiculos.aseguradora a on c.id_aseguradora = a.id
join flotavehiculos.revision r on c.id = r.id_carro
join flotavehiculos.marca m on c.id_marca = m.id
join flotavehiculos.grupo_empresarial ge on m.id_grupo = ge.id